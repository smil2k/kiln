...
// Mega does not have in on PORTC and D
...
public:
  ...
  static void attachInterruptByMask(uint8_t port, uint8_t mask, PCIntvoidFuncPtr userFunc, int mode);
...
protected:
  ...
  void addPin(uint8_t mask, uint8_t mode, PCIntvoidFuncPtr userFunc);
...
#define PORTB 2
PCintPort portB = PCintPort(PORTB, 0, PCMSK0); // port PB==2  (from Arduino.h, Arduino version 1.0)
#endif

#ifndef NO_PORTC_PINCHANGES
#define PORTC 3
PCintPort portC = PCintPort(PORTC, 1, PCMSK1); // port PC==3  (also in pins_arduino.c, Arduino version 022)
#endif

#ifndef NO_PORTD_PINCHANGES
#define PORTD 4
PCintPort portD = PCintPort(PORTD, 2, PCMSK2); // port PD==4
#endif
...
void PCintPort::addPin(uint8_t mask, uint8_t mode, PCIntvoidFuncPtr userFunc) {
  // Create pin p:  fill in the data.  This is no longer in another method (saves some bytes).
  PCintPin* p = new PCintPin;
  p->mode = mode;
  p->next = NULL;
  p->mask = mask; // the mask

#ifdef DEBUG
  Serial.print("createPin. pin given: ");
...
void PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc, int mode = RISING) {
  attachInterruptByMask(digitalPinToPort(arduinoPin), digitalPinToBitMask(arduinoPin), userFunc, mode);
}

/*
 * attach an interrupt to a specific pin using pin change interrupts.
 */
void PCintPort::attachInterruptByMask(uint8_t portNum, uint8_t mask, PCIntvoidFuncPtr userFunc, int mode = RISING) {
  if ((portNum == NOT_A_PORT) || (userFunc == NULL)) return;

  PCintPort * port = lookupPortNumToPort(portNum);
  if (port != NULL) {
    ...
    port->addPin(mask, mode, userFunc);
  }
}
...
ISR(PCINT1_vect) {
  ...
  PCintPort::curr = portJ.portInputReg; // version 1.6
  portJ.PCint();
}
#endif
...
ISR(PCINT2_vect) {
  PCintPort::curr = portK.portInputReg; // version 1.6
  portK.PCint();
}
#endif
